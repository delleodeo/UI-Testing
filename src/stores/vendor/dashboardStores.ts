...
export const useVendorDashboardStore = defineStore("vendor", {
  ...
  actions: {
    ...
    async fetchVendor(sortDir: SortDirection = "desc") {
      ...
      this.error = null;
      try {
        const { data: vendorData } = await axios.get(`${API_BASE_URL}/vendor`, {
          headers,
        });
        ...
        const normalized: Product[] = rawProducts.map((p: any) =>
    ...
    async updateBaseProduct(productId: string, updated: Partial<Product>) {
      try {
        await axios.put(`${API_BASE_URL}/products/${productId}`, updated, {
          headers,
        });
        ...
        Alert(
          `Product update failed! ${error?.response?.status || ""}`,
          "error",
          "var(--secondary-color)"
        );
        console.error(error);
    ...
    async deleteProduct(productId: string, variantId: string) {
      try {
        ...
        if (deletingVariant) {
          ...
          this.vendorProducts = this.vendorProducts.filter(
            (p) => p._id !== productId
          );
        }
    ...
    /* ------------------ Update Option ------------------ */
    async updateOptionedProduct(
      productId: string,
      variantId: string,
      updated: Partial<ProductOption>
    ) {
      try {
        ...
        if (updated.imageUrl && isDataUrl(updated.imageUrl)) {
          try {
            const resp = await uploadImageDataUrl(
              updated.imageUrl,
              productId,
              variantId
            );
            ...
            Alert(
              "Image upload failed. Update aborted.",
              "error",
              "var(--secondary-color)"
            );
        ...
        const productIdx = this.vendorProducts.findIndex(
          (p) => p._id === productId
        );
        if (productIdx > -1) {
        ...
        Alert(
          `Product option update failed! ${error?.response?.status || ""}`,
          "error",
          "var(--secondary-color)"
        );
        console.error(error);
    ...
    async addOption(productId: string, newOption: NewOptionInput) {
      ...
      if (newOption.imageUrl && isDataUrl(newOption.imageUrl)) {
        try {
          ...
          if (!finalUrl) {
            Alert(
              "Option image upload failed (no URL).",
              "error",
              "var(--secondary-color)"
            );
            return;
